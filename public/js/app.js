...
if (commentsSectionIni.length && commentsSectionIni.data("href")) {
  axios.get(commentsSectionIni.data("href")).then(function (res) {
    commentsSectionIni.after($(res.data)); //commentsSection.append( $(res.data) );
  ...
  $(document).on("submit", function (e) {
    try {
      ...
      var form = $(e.target);
      var comments = $(form.data("target"));
      var loadDown = comments.find("._load-down")[0];
      var content = form.find("[name='content']")[0];
      ...
      axios.post(loadDown.dataset.href, {
        ...
        console.log(comments); // form.find load-down and delete it
    ...
    return false;
    var commentsSection = $(e.target).parent();
...
if (commentsContainer.length) {
  var src = commentsContainer.data("src");
  $('#comments-container').comments({
    ...
    postComment: function postComment(commentJSON, success, error) {
      axios.post(src, commentJSON).then(function (res) {
        success(res.data);
      });
    },
    fieldMappings: {
...
$('oembed').each(function (i, el) {
  console.log(el);
  var media = $(el);
  var mediaUrl = $(el).attr('url');
  media.append("<figure class=\"relative w-full\" style=\"padding-bottom:75%;\"><iframe class=\"absolute w-full h-full\" src=\"".concat(mediaUrl, "\" frameborder=\"0\" allow=\"accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture\" allowfullscreen></iframe></figure>"));
});
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _jquery_comments__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./jquery-comments */ "./resources/js/jquery-comments.js");
/* harmony import */ var _jquery_comments__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_jquery_comments__WEBPACK_IMPORTED_MODULE_0__);
...
/* harmony import */ var js_autocomplete_auto_complete__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! js-autocomplete/auto-complete */ "./node_modules/js-autocomplete/auto-complete.js");
/* harmony import */ var js_autocomplete_auto_complete__WEBPACK_IMPORTED_MODULE_3___default = /*#__PURE__*/__webpack_require__.n(js_autocomplete_auto_complete__WEBPACK_IMPORTED_MODULE_3__);
// window._ = require('lodash');
//import modal from "jquery-modal";
window.axios = __webpack_require__(/*! axios */ "./node_modules/axios/index.js");
window.$ = window.jquery = window.jQuery = __webpack_require__(/*! jquery */ "./node_modules/jquery/dist/jquery.js"); // import 'jquery-ui/ui/widgets/tabs.js';

__webpack_require__(/*! jquery-modal */ "./node_modules/jquery-modal/jquery.modal.js");



__webpack_require__(/*! ./components/input-char-count */ "./resources/js/components/input-char-count.js");


window.AWN = awesome_notifications__WEBPACK_IMPORTED_MODULE_1___default.a; // window.popper = require('popper.js');
// require('bootstrap');
...
$('.tagify-input').each(function (i, el) {
  ...
  var tagify = new Tagify(el, {
    whitelist: whitelist,
    pattern: pattern
  }).on('input', lodash_debounce__WEBPACK_IMPORTED_MODULE_2___default()(function (e) {
...
$('.autocomplete-input').each(function (i, el) {
  ...
  new js_autocomplete_auto_complete__WEBPACK_IMPORTED_MODULE_3___default.a({
    delay: 500,
    selector: el,
    minChars: 1,
...
/***/ "./resources/js/components/input-char-count.js":
/*!*****************************************************!*\
  !*** ./resources/js/components/input-char-count.js ***!
  \*****************************************************/
/*! no static exports found */
...
/***/ "./resources/js/jquery-comments.js":
/*!*****************************************!*\
  !*** ./resources/js/jquery-comments.js ***!
  \*****************************************/
/*! no static exports found */
  ...
  var Comments = {
    ...
    getDefaultOptions: function getDefaultOptions() {
      return {
        ...
        searchUsers: function searchUsers(term, success, error) {
          success([]);
        },
        getComments: function getComments(success, error) {
          success([]);
    ...
    init: function init(options, el) {
      ...
      this.delegateEvents(); // Detect mobile devices
      ...
      if ($.browser.mobile) this.$el.addClass('mobile'); // Init options

      this.options = $.extend(true, {}, this.getDefaultOptions(), options);
      ; // Read-only mode

      if (this.options.readOnly) this.$el.addClass('read-only'); // Set initial sort key

      this.currentSortKey = this.options.defaultNavigationSortKey; // Create CSS declarations for highlight color

      this.createCssDeclarations(); // Fetching data and rendering
      ...
      for (var key in this.events) {
        ...
        for (var index in methodNames) {
          if (methodNames.hasOwnProperty(index)) {
            var method = this[methodNames[index]]; // Keep the context
    ...
    fetchDataAndRender: function fetchDataAndRender() {
      ...
      this.options.getComments(function (commentsArray) {
        ...
        self.dataFetched = true; // Render

        self.render();
      });
    },
    fetchNext: function fetchNext() {
      var self = this; // Loading indicator
    ...
    createCommentModel: function createCommentModel(commentJSON) {
      // NOTE - applyInternalMappings inlatura anumite proprietati din commentJson
    ...
    addCommentToDataModel: function addCommentToDataModel(commentModel) {
      if (!(commentModel.id in this.commentsById)) {
        this.commentsById[commentModel.id] = commentModel; // Update child array of the parent (append childs to the array of outer most parent)
    ...
    render: function render() {
      var self = this; // Prevent re-rendering if data hasn't been fetched

      if (!this.dataFetched) return; // Show active container

      this.showActiveContainer(); // Create comments

      this.createComments(); // Create attachments if enabled

      if (this.options.enableAttachments) this.createAttachments(); // Remove spinner
    ...
    createComments: function createComments() {
      var self = this; // Create the list element before appending to DOM in order to reach better performance
      ...
      this.sortComments(mainLevelComments, this.currentSortKey); // EDIT - am adaugat urmatoarea verificare pentru ca revers-ul facea ca default - oldest sa nu mearga

      if (this.options.defaultNavigationSortKey != "oldest") mainLevelComments.reverse(); // Reverse the order as they are prepended to DOM
    ...
    addComment: function addComment(commentModel, commentList) {
      ...
      var commentEl = this.createCommentElement(commentModel); // Case: reply

      if (commentModel.parent) {
        var directParentEl = commentList.find('.comment[data-id="' + commentModel.parent + '"]'); // Re-render action bar of direct parent element

        this.reRenderCommentActionBar(commentModel.parent); // Force replies into one level only

        var outerMostParent = directParentEl.parents('.comment').last();
        if (outerMostParent.length == 0) outerMostParent = directParentEl; // Append element to DOM
        ...
        this.updateToggleAllButton(outerMostParent); // Case: main level comment
      } else {
    ...
    removeComment: function removeComment(commentId) {
      var self = this;
      var commentModel = this.commentsById[commentId]; // Remove child comments recursively
      ...
      var parentEl = commentElements.parents('li.comment').last(); // Remove the element

      commentElements.remove(); // Update the toggle all button
    ...
    uploadAttachments: function uploadAttachments(files, commentingField) {
      ...
      if (fileCount) {
        var textarea = commentingField.find('.textarea'); // Disable upload button and append spinners while request is pending
        ...
        $(files).each(function (index, file) {
          ...
          commentJSON.fileMimeType = file.type; // Reverse mapping
    ...
    updateToggleAllButton: function updateToggleAllButton(parentEl) {
      ...
      childComments.removeClass('togglable-reply'); // Select replies to be hidden
      ...
      togglableReplies.addClass('togglable-reply'); // Show all replies if replies are expanded
      ...
      if (childComments.length > this.options.maxRepliesVisible) {
        ...
        this.setToggleAllButtonText(toggleAllButton, false); // Make sure that toggle all button is not present
      } else {
        toggleAllButton.remove();
      }
    },
    updateToggleAllButtons: function updateToggleAllButtons() {
      var self = this;
      var commentList = this.$el.find('#comment-list'); // Fold comments, find first level children and update toggle buttons
    ...
    sortComments: function sortComments(comments, sortKey) {
      var self = this; // Sort by popularity
    ...
    sortAndReArrangeComments: function sortAndReArrangeComments(sortKey) {
      var commentList = this.$el.find('#comment-list'); // Get main level comments

      var mainLevelComments = this.getComments().filter(function (commentModel) {
        return !commentModel.parent;
      });
      this.sortComments(mainLevelComments, sortKey); // Rearrange the main level comments
    ...
    showActiveSort: function showActiveSort() {
      var activeElements = this.$el.find('.navigation li[data-sort-key="' + this.currentSortKey + '"]'); // Indicate active sort

      this.$el.find('.navigation li').removeClass('active');
      activeElements.addClass('active'); // Update title for dropdown
    ...
    checkEditableContentForChange: function checkEditableContentForChange(ev) {
      var el = $(ev.currentTarget); // Fix jquery-textcomplete on IE, empty text nodes will break up the autocomplete feature
    ...
    navigationElementClicked: function navigationElementClicked(ev) {
      ...
      var sortKey = navigationEl.data().sortKey; // Sort the comments if necessary
    ...
    textareaContentChanged: function textareaContentChanged(ev) {
      var textarea = $(ev.currentTarget);
      var saveButton = textarea.siblings('.control-row').find('.save'); // Update parent id if reply-to tag was removed

      if (!textarea.find('.reply-to.tag').length) {
        var commentId = textarea.attr('data-comment'); // Case: editing comment
    ...
    removeCommentingField: function removeCommentingField(ev) {
      var closeButton = $(ev.currentTarget); // Remove edit class from comment if user was editing the comment
    ...
    postComment: function postComment(ev) {
      ...
      var textarea = commentingField.find('.textarea'); // Disable send button while request is pending

      sendButton.removeClass('enabled'); // Create comment JSON

      var commentJSON = this.createCommentJSON(textarea); // Reverse mapping
    ...
    // NOTE - this.createCommentModel inlatura poza de profil...
    ...
    putComment: function putComment(ev) {
      ...
      saveButton.removeClass('enabled'); // Use a clone of the existing model and update the model after succesfull update
      ...
      var success = function success(commentJSON) {
        ...
        var commentModel = self.createCommentModel(commentJSON); // Delete childs array from new comment model since it doesn't need an update

        delete commentModel['childs'];
        self.updateCommentModel(commentModel); // Close the editing field

        commentingField.find('.close').trigger('click'); // Re-render the comment
    ...
    deleteComment: function deleteComment(ev) {
      ...
      var parentId = commentJSON.parent; // Disable send button while request is pending

      deleteButton.removeClass('enabled'); // Reverse mapping
    ...
    upvoteComment: function upvoteComment(ev) {
      ...
      var commentModel = commentEl.data().model; // Check whether user upvoted the comment or revoked the upvote
      ...
      this.reRenderUpvotes(commentModel.id); // Reverse mapping
    ...
    replyButtonClicked: function replyButtonClicked(ev) {
      ...
      var parentId = replyButton.parents('.comment').first().data().id; // Remove existing field

      var replyField = outermostParent.find('.child-comments > .commenting-field');
      if (replyField.length) replyField.remove();
      var previousParentId = replyField.find('.textarea').attr('data-parent'); // Create the reply field (do not re-create)

      if (previousParentId != parentId) {
        replyField = this.createCommentingFieldElement(parentId);
        outermostParent.find('.child-comments').append(replyField); // Move cursor to end

        var textarea = replyField.find('.textarea');
        this.moveCursorToEnd(textarea); // Make sure the reply field will be displayed
    ...
    editButtonClicked: function editButtonClicked(ev) {
      ...
      commentEl.addClass('edit'); // Create the editing field

      var editField = this.createCommentingFieldElement(commentModel.parent, commentModel.id);
      commentEl.find('.comment-wrapper').first().append(editField); // Append original content

      var textarea = editField.find('.textarea');
      textarea.attr('data-comment', commentModel.id); // Escaping HTML

      textarea.append(this.getFormattedCommentContent(commentModel, true)); // Move cursor to end
    ...
    handleDrop: function handleDrop(ev) {
      ev.preventDefault(); // Reset DND counts

      $(ev.target).trigger('dragleave'); // Hide the overlay and upload the files
    ...
    createHTML: function createHTML() {
      var self = this; // Commenting field

      var mainCommentingField = this.createMainCommentingFieldElement();
      this.$el.append(mainCommentingField); // Hide control row and close button

      var mainControlRow = mainCommentingField.find('.control-row');
      mainControlRow.hide();
      mainCommentingField.find('.close').hide(); // Navigation bar
      ...
      var spinner = this.createSpinner();
      this.$el.append(spinner); // Comments container

      var commentsContainer = $('<div/>', {
        'class': 'data-container',
        'data-container': 'comments'
      });
      this.$el.append(commentsContainer); // "No comments" placeholder
      ...
      commentsContainer.append(noComments); // Attachments
      ...
      if (this.options.enableAttachments) {
        ...
        this.$el.append(attachmentsContainer); // "No attachments" placeholder
        ...
        attachmentsContainer.append(noAttachments); // Drag & dropping attachments
    ...
    createProfilePictureElement: function createProfilePictureElement(src, userId) {
      if (src) {
    ...
    createMainCommentingFieldElement: function createMainCommentingFieldElement() {
      return this.createCommentingFieldElement(undefined, undefined, true);
    },
    createCommentingFieldElement: function createCommentingFieldElement(parentId, existingCommentId, isMain) {
      var self = this; // Commenting field

      var commentingField = $('<div/>', {
        'class': 'commenting-field'
      });
      if (isMain) commentingField.addClass('main'); // Comment was modified, use existing data

      if (existingCommentId) {
        var profilePictureURL = this.commentsById[existingCommentId].profilePictureURL;
        var userId = this.commentsById[existingCommentId].creator; // New comment was created
      } else {
        var profilePictureURL = this.options.profilePictureURL;
        var userId = this.options.creator;
      }

      var profilePicture = this.createProfilePictureElement(profilePictureURL, userId); // New comment
      ...
      this.adjustTextareaHeight(textarea, false); // Close button

      var closeButton = $('<span/>', {
        'class': 'close inline-button'
      }).append($('<span class="left"/>')).append($('<span class="right"/>')); // Save button text

      if (existingCommentId) {
        var saveButtonText = this.options.textFormatter(this.options.saveText); // Delete button

        var deleteButton = $('<span/>', {
          'class': 'delete',
          text: this.options.textFormatter(this.options.deleteText)
        }).css('background-color', this.options.deleteButtonColor);
        controlRow.append(deleteButton); // Enable the delete button only if the user is allowed to delete

        if (this.isAllowedToDelete(existingCommentId)) deleteButton.addClass('enabled');
      } else {
        var saveButtonText = this.options.textFormatter(this.options.sendText); // Add upload button if attachments are enabled
        ...
        if (this.options.enableAttachments) {
          ...
          uploadButton.append(uploadIcon).append(fileInput); // Main upload button

          controlRow.append(uploadButton.clone()); // Inline upload button for main commenting field
      ...
      if (parentId) {
        ...
        textarea.attr('data-parent', parentId); // Append reply-to tag if necessary
      ...
      if (this.options.enablePinging) {
        textarea.textcomplete([{
          ...
          search: function search(term, callback) {
            term = self.normalizeSpaces(term); // Return empty array on error
        ...
        $.fn.textcomplete.Dropdown.prototype.render = function (zippedData) {
          ...
          this.$el.css('top', top); // Adjust horizontal position
    ...
    createNavigationElement: function createNavigationElement() {
      ...
      navigationEl.append(navigationWrapper); // Newest
      ...
      attachments.prepend(attachmentsIcon); // Responsive navigation
      ...
      navigationEl.append(dropdownNavigationWrapper); // Populate elements
    ...
    createCommentElement: function createCommentElement(commentModel) {
      ...
      if (commentModel.createdByAdmin) commentEl.addClass('by-admin'); // Child comments
    ...
    createCommentWrapperElement: function createCommentWrapperElement(commentModel) {
      ...
      var profilePicture = this.createProfilePictureElement(commentModel.profilePictureURL, commentModel.creator); // Time
      ...
      if (commentModel.createdByAdmin) nameEl.addClass('highlight-font-bold'); // Show reply-to name if parent of parent exists
      ...
      if (isAttachment) {
        ...
        if (type == 'image') {
          ...
          link.html(image); // Case: video preview
        } else if (type == 'video') {
          var video = $('<video/>', {
            src: commentModel.fileURL,
          ...
          link.html(video); // Case: icon and text
        } else {
          // Icon
        ...
        content.html(link); // Case: regular comment
      } else {
      ...
      var upvotes = this.createUpvoteElement(commentModel); // Append buttons for actions that are enabled
      ...
      if (commentModel.createdByCurrentUser || this.options.currentUserIsAdmin) {
        ...
        if (isAttachment && this.isAllowedToDelete(commentModel.id)) {
          ...
          actions.append(deleteButton); // Case: edit button for regular comment
    ...
    createTagElement: function createTagElement(text, extraClasses, value, extraAttributes) {
      var tagEl = $('<input/>', {
        'class': 'tag',
        'type': 'button',
        'data-role': 'none'
    ...
    createCssDeclarations: function createCssDeclarations() {
      ...
      $('head style.jquery-comments-css').remove(); // Navigation underline

      this.createCss('.jquery-comments ul.navigation li.active:after {background: ' + this.options.highlightColor + ' !important;', +'}'); // Dropdown active element

      this.createCss('.jquery-comments ul.navigation ul.dropdown li.active {background: ' + this.options.highlightColor + ' !important;', +'}'); // Background highlight

      this.createCss('.jquery-comments .highlight-background {background: ' + this.options.highlightColor + ' !important;', +'}'); // Font highlight
    ...
    getTextareaContent: function getTextareaContent(textarea, humanReadable) {
      var textareaClone = textarea.clone(); // Remove reply-to tag

      textareaClone.find('.reply-to.tag').remove(); // Replace tags with text values
      ...
      var text = ce.text().replace(/^\s+/g, ''); // Normalize spaces
    ...
    moveCursorToEnd: function moveCursorToEnd(el) {
      el = $(el)[0]; // Trigger input to adjust size

      $(el).trigger('input'); // Scroll to bottom

      $(el).scrollTop(el.scrollHeight); // Move cursor to end
    ...
    normalizeSpaces: function normalizeSpaces(inputText) {
      return inputText.replace(new RegExp("\xA0", 'g'), ' '); // Convert non-breaking spaces to reguar spaces
    },
    ...
    highlightPings: function highlightPings(commentModel, html) {
      ...
      if (html.indexOf('@') != -1) {
        var __createTag = function __createTag(pingText, userId) {
    ...
    linkify: function linkify(inputText) {
      var replacedText, replacePattern1, replacePattern2, replacePattern3; // URLs starting with http://, https://, ftp:// or file://

      replacePattern1 = /(\b(https?|ftp|file):\/\/[-A-ZÄÖÅ0-9+&@#\/%?=~_|!:,.;]*[-A-ZÄÖÅ0-9+&@#\/%=~_|])/gim;
      replacedText = inputText.replace(replacePattern1, '<a href="$1" target="_blank">$1</a>'); // URLs starting with "www." (without // before it, or it would re-link the ones done above).

      replacePattern2 = /(^|[^\/f])(www\.[-A-ZÄÖÅ0-9+&@#\/%?=~_|!:,.;]*[-A-ZÄÖÅ0-9+&@#\/%=~_|])/gim;
      replacedText = replacedText.replace(replacePattern2, '$1<a href="https://$2" target="_blank">$2</a>'); // Change email addresses to mailto: links.

      replacePattern3 = /(([A-ZÄÖÅ0-9\-\_\.])+@[A-ZÄÖÅ\_]+?(\.[A-ZÄÖÅ]{2,6})+)/gim;
      replacedText = replacedText.replace(replacePattern3, '<a href="mailto:$1" target="_blank">$1</a>'); // If there are hrefs in the original text, let's split
    ...
    applyInternalMappings: function applyInternalMappings(commentJSON) {
      ...
      } // NOTE - applyMappings inlatura ceva proprietati de fapt
